...
5. Repository URL: `<your_repository_url>/SimpleInfrastructureStack.git`
...
10. Verification:
    - Check the logs for the `traefik` container to ensure it starts without errors and detects the Docker provider.
    - Check the logs for the `cloudflare-ddns` container to confirm it is successfully updating your DNS records.
    - After a few minutes, navigate to `https://traefik.your-domain.com`. You should receive a `404 page not found` error from Traefik, but with a valid SSL certificate issued by Let's Encrypt. This is expected behavior as Authentik is not yet running to handle the request.
